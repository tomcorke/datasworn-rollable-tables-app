...
const tables = Object.entries(bundledData).flatMap(([key, data]) => {
  const ruleset = Object.keys(rulesets).find(name => key.startsWith(`${name}-`));
  return getTables(data).map(t => ({ ...t, sourceKey: key.slice(`${ruleset}-`.length), ruleset }));
});
...
const unresolved = tableReferences.filter(reference => !resolveTableLink(reference.id, tables));
...
describe('bundled Datasworn links', () => {
  ...
  it('resolves every reference whose target data is bundled', () => {
    const missing = unresolved.filter(({ id }) => !unavailableCollections.has(id.split('/').at(-1)));
  ...
  it('does not cross-resolve rulesets with the same short link', () => {
    const sundered = { id: 'sailing_ships/contents/size', sourceKey: 'sailing_ships', ruleset: 'sundered_isles', tableId: 'size' };
    const starforged = { id: 'starships/contents/size', sourceKey: 'starships', ruleset: 'starforged', tableId: 'size' };
    expect(resolveTableLink('sundered_isles/collections/oracles/ships', [sundered, starforged])).toBe(sundered);
  });
  it('reports non-table and unavailable references explicitly', () => {
    expect(references.filter(reference => nonTableTargets.test(reference.id)).length).toBeGreaterThan(0);
    expect(unresolved.map(reference => reference.id)).toContain('sundered_isles/collections/oracles/ruins');
  });
});
